...
var UhkConnection = function(selectedLogLevel) {
    ...
    self.connect = function(callback) {
        device = usb.findByIds(UhkConnection.VENDOR_ID, UhkConnection.PRODUCT_IDS.KEYBOARD_6KRO);
        if (!device) {
            return callback('Could not connect to the Ultimate Hacking Keyboard. Is it connected to the host?');
        }
...
UhkConnection.PRODUCT_IDS = {
    KEYBOARD_6KRO:    0x05ea,
    KEYBOARD_NKRO:    0x05eb,
    BOOTLOADER_RIGHT: 0x05ec,  // CDC bootloader
    BOOTLOADER_LEFT:  0x05ed   // USB to serial
};
...
UhkConnection.ENUMERATION_MODES = {
    KEYBOARD:         0,
    BOOTLOADER_RIGHT: 1,  // CDC bootloader
    BOOTLOADER_LEFT:  2   // USB to serial
};
