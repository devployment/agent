...
var UhkConnection = function() {
    ...
    scope.sendRequest = function(command, arg, callback) {
        var request = new Buffer([command, arg]);
...
//        console.log('endpoints', usbInterface.endpoints);
    ...
    scope.connect = function(callback) {
        ...
        usbInterface = device.interface(UhkConnection.GENERIC_HID_INTERFACE_ID);
        if (usbInterface.isKernelDriverActive()) {
            usbInterface.detachKernelDriver();
        }

        usbInterface.claim();
        setConfiguration(0, function(data, error) {
            callback(data, error);
        });
    };
};
