...
var UhkConnection = function(selectedLogLevel) {
    ...
    scope.sendRequest = function(command, arg, callback) {
        ...
        if (arg === null) {
            ...
            request = new Buffer([command].concat(arg.split('').map(function(char) {return char.charCodeAt(0)})));
        }

        log(UhkConnection.LOG_LEVELS.TRANSFER, 'Sending request', request);
        setReport(request, function(error, data) {
            receiveResponse(UhkConnection.LOG_LEVELS.IGNORED_TRANSFER, function() {  // The first response is cached so let's discard it.
